...
from tools.alarm import create_alarm, cancel_alarm, list_alarms
...
def handle_alarm(payload):
    alarm_time, description = split_payload(payload, 2)
    return create_alarm(alarm_time,description)

def handle_email(payload):
    email_address, query = split_payload(payload, 2)
    return check_email(email_address,query)

def handle_list_alarm(payload):
    return list_alarms()
